"""
Code from https://github.com/pytorch/pytorch/issues/41508#issuecomment-1723119580

PyTorch attention has a bug that results in nan gradients in the prescence of -inf attention logits.
The following reimplementations address this.
"""
...
def _scaled_dot_product_attention(
    ...
    if attn_mask is not None:
        attn.masked_fill_(attn_mask.to(torch.bool).to(device = attn.device), float('-inf'))
    attn = F.softmax(attn, dim=-1)
    if attn_mask is not None:
        attn = attn.masked_fill(attn_mask.to(torch.bool).to(device = attn.device), 0.0)
    if dropout_p > 0.0:
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
                ...
                tgt_is_causal: Optional[bool] = None,  memory_is_causal: Optional[bool] = None) -> Tensor:
